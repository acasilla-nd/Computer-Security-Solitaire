...
ts=["t0","t1","t2","t3"]
breaklim=50000 #heuristic for cutting off the algorithm after searching a certain number of states, signifying that the given setup is likely impossible
...
def make_state(deck,piles,top): #for hashing only. Otherwise the arrays are easier to use
    ...
    for pile in piles:
        if(not pile):
            state+="~"
        #only putting hidden cards in the pile into the state because the locations of the other cards actually doesn't matter based on the moves we are doing, but i did have to design it that way
        state+=" ".join([str(x) for x in pile if (1<<6)&x])+"&"
    state=state[:-1]+"&"
    for t in top:
...
def legal_moves(deck,piles,top,deckind):
    moves=[]
    
    #instead of making every shuffle a move, we just look at the whole thing at once. Much faster this way
    for j in range(len(deck)):
        i=(deckind-j-int(deckind==len(deck)))%len(deck) #heuristic, searching for valid moves from deck prioritizing ones that are fewer flips away
    ...
    #okay yeah we don't need to check pulling stuff off the top. Doesn't help
...
def make_move(move,deck,piles,top,deckind): #only legal moves should be called here, otherwise an error likely will occur
    if(move[0][0][0]=="p"): #moving from pile, might have to reveal card underneath
        ...
        if(move[1][0][0]=="t"): #to top
            ...
            piles[ind2].append(card)
        deckind=ind
    return (deck,piles,top,deckind)
    
def solve(deck,piles,top,deckind=24,memo={}):
    ...
    if(shash in memo):
        return memo[shash]
    memo[shash]=([],False)
    if(len(memo)>breaklim):
        return ([],False)
    sol=[]
    for move in legal_moves(deck,piles,top,deckind):
        #deref all for temp variables for recursion
        tdeck=list(deck)
        tpiles=copy.deepcopy(piles)
        ttop=copy.deepcopy(top)
        tdeck,tpiles,ttop,tdeckind=make_move(move,tdeck,tpiles,ttop,deckind)
        thash=make_state(tdeck,tpiles,ttop)
        if(thash in memo):
            continue
        r1,r2=solve(tdeck,tpiles,ttop,tdeckind,memo)
...
def expand_moves(moves,deck,piles,top,deckind=24):
    newmoves=[]
    for i in range(len(moves)):
        ...
        if(move[0][0]=="d"):
            ind=move[0][1]
            if(deckind<ind):
                shuffle=1+deckind+len(deck)-ind
            else:
                shuffle=deckind-ind
            for j in range(shuffle):
                newmoves.append((("d",0),("d",0)))
            newmoves.append((("d",0),move[1]))
        else:
            newmoves.append(move)
        deck,piles,top,deckind=make_move(move,deck,piles,top,deckind)
    return newmoves
